...
//process.env.BEDROCK_OPENAI_BASE_URL="https://bedrock-runtime.us-east-1.amazonaws.com/openai/v1";
//process.env.BEDROCK_MODEL="us.openai.gpt-5.6-luna";
//process.env.AWS_BEARER_TOKEN_BEDROCK="your-bedrock-api-key";
...
if (process.env.APPLICATION_PORT) {
  console.log(
    "Setting process.env.APPLICATION_PORT from envars:",
    process.env.APPLICATION_PORT,
  );
} else {
  process.env.APPLICATION_PORT = "8080";
  console.log(
    "Using default process.env.APPLICATION_PORT:",
    process.env.APPLICATION_PORT,
  );
}
...
if (process.env.APPLICATION_HOST) {
  console.log(
    "Setting process.env.APPLICATION_HOST from envars:",
    process.env.APPLICATION_HOST,
  );
} else {
  process.env.APPLICATION_HOST = "127.0.0.1";
  console.log(
    "Using default process.env.APPLICATION_HOST:",
    process.env.APPLICATION_HOST,
  );
}

if (process.env.BEDROCK_OPENAI_BASE_URL) {
  console.log(
    "Setting process.env.BEDROCK_OPENAI_BASE_URL from envars:",
  ...
  console.log(
    "Using default process.env.BEDROCK_OPENAI_BASE_URL:",
    process.env.BEDROCK_OPENAI_BASE_URL,
  );
}
...
if (process.env.BEDROCK_MODEL) {
  console.log(
    "Setting process.env.BEDROCK_MODEL from envars:",
    process.env.BEDROCK_MODEL,
  );
} else {
  process.env.BEDROCK_MODEL = "us.openai.gpt-5.6-luna";
  console.log(
    "Using default process.env.BEDROCK_MODEL:",
    process.env.BEDROCK_MODEL,
  );
}
...
if (process.env.ENABLE_RAG_LLM) {
  console.log(
    "Using process.env.ENABLE_RAG_LLM from envars:",
    process.env.ENABLE_RAG_LLM,
  );
  if (process.env.ENABLE_RAG_LLM.toLowerCase() === "true") {
    console.log(
      "IBM-hosted RAG is enabled and requires WATSONX_RISK_RAG_LLM_ENDPOINT and WATSONX_AI_APIKEY.",
    );
    if (
      !process.env.WATSONX_RISK_RAG_LLM_ENDPOINT ||
      !process.env.WATSONX_AI_APIKEY
    ) {
      throw new Error(
        "ENABLE_RAG_LLM=true requires WATSONX_RISK_RAG_LLM_ENDPOINT and WATSONX_AI_APIKEY.",
      );
    }
  }
} else {
  ...
  console.log(
    "Using default process.env.ENABLE_RAG_LLM:",
    process.env.ENABLE_RAG_LLM,
  );
}
...
if (process.env.ENABLE_WXASST) {
  if (process.env.ENABLE_WXASST.toLowerCase() === "true") {
    console.log(
      "Setting up watsonx Assistant widget for page /wx.html. Envar ENABLE_WXASST:",
      process.env.ENABLE_WXASST,
    );
    if (
      process.env.WXASST_INTEGRATION_ID &&
      process.env.WXASST_REGION &&
      process.env.WXASST_SERVICE_INSTANCE_ID
    ) {
      ...
      console.log(
        "Unable to set up watsonx Assistant widget. Missing one or more envars from WXASST_INTEGRATION_ID, WXASST_REGION, WXASST_SERVICE_INSTANCE_ID,",
      );
    }
  }
}
...
const agentic_instructions = {
  credit_score_tool:
    "Get the credit score for the customer using the customer id. Customer's name can be used instead of customer's id. If the credit score is already known do not retrieve again.",
  account_status_tool:
    "Get the account status for the customer using customer id. Customer's name can be used instaed of customer's id. If the account status is already known do not retrieve again.",
  overall_risk_tool:
  ...
  overall_risk_from_rag_llm_tool:
    "Get overall risk based on combination of both credit score and account status. Explain how the overall risk was calculated. If the credit score and account status are not known then do not provide the risk status and first retrieve the missing credit score or account status.",
  overall_risk_from_rag_llm_tool_prompt:
    "what is the risk for credit score {credit_score} and account status {account_status}, and how is it determined?",
  interest_rate_tool:
    "Get interest rate percentage based on overall risk. Explain how the interest rate was determined. If the overall risk is not known then do not provide the interest rate status and first retrieve the overall risk.",
  interest_rate_from_rag_llm_tool_prompt:
    "what is the interest rate for overall risk {overall_risk} and how was it determined?",
  interest_rate_from_rag_llm_tool:
...
const makePostRequestCallByType = async (
  calltype: string,
  post_data: any,
): Promise<any> => {
  ...
  try {
    ...
    if (
      calltype === "get_risk_from_rag_llm" ||
      calltype === "get_interest_rate_from_rag_llm"
    ) {
...
const get_ibm_iam_token = async () => {
  ...
  if (
    process.env.IBM_IAM_TOKEN_EXPIRATION === undefined ||
    process.env.IBM_IAM_TOKEN_EXPIRATION === null ||
    parseInt(process.env.IBM_IAM_TOKEN_EXPIRATION) < Date.now() / 1000 - 300
  ) {
    // Token has expired
    console.log(
      "Token has expired process.env.IBM_IAM_TOKEN_EXPIRATION, Date.now()/1000 :",
      process.env.IBM_IAM_TOKEN_EXPIRATION,
      Date.now() / 1000,
    );
    ...
    console.log(
      "Token is valid. process.env.IBM_IAM_TOKEN_EXPIRATION, Date.now()/1000 :",
      process.env.IBM_IAM_TOKEN_EXPIRATION,
      Date.now() / 1000,
    );
...
const setupTools = async () => {
  ...
  const getCreditScore = tool(
    ({ customer_id }) => {
      var credit_score = 0;
      customer_id = customer_id.toLowerCase();
      if (
        customer_id === "loren@ibm.com" ||
        customer_id === "loren" ||
        customer_id === "1111"
      ) {
        credit_score = 455;
      } else if (
        customer_id === "matt@ibm.com" ||
        customer_id === "matt" ||
        customer_id === "2222"
      ) {
        credit_score = 685;
      } else if (
        customer_id === "hilda@ibm.com" ||
        customer_id === "hilda" ||
        customer_id === "3333"
      ) {
        credit_score = 825;
  ...
  const getOverallRiskFromRAGLLM = tool(
    async ({ credit_score, account_status }) => {
      //Call the deployed RAG LLM API endpoint with above query/message content and the the overall risk description.
      const risk_rag_llm_query =
        agentic_instructions.overall_risk_from_rag_llm_tool_prompt
          .replace("{credit_score}", credit_score.toString())
          .replace("{account_status}", account_status);

      //console.log("risk_rag_llm_query:", risk_rag_llm_query);
      //const payload = '{"messages": [{ "role": "user", "content": "' + risk_rag_llm_query + '" }]}';
      const risk_rag_llm_request_obj = {
        messages: [{ role: "user", content: risk_rag_llm_query }],
      };
      const risk_rag_llm_request = JSON.stringify(risk_rag_llm_request_obj);
      console.log("payload:", risk_rag_llm_request);

      const risk_rag_llm_response = await makePostRequestCallByType(
        "get_risk_from_rag_llm",
        risk_rag_llm_request,
      )
        .then((responseData) => {
          console.log("Response:", responseData);
          return responseData;
      ...
      console.log(
        "risk_rag_llm_response.choices[0].message.content:",
        risk_rag_llm_response.choices[0].message.content,
      );
  ...
  const getInterestRateFromRAGLLM = tool(
    async ({ overall_risk }) => {
      //Call the deployed RAG LLM API endpoint with above query/message content and the interest rate description.
      const interest_rate_rag_llm_query =
        agentic_instructions.interest_rate_from_rag_llm_tool_prompt.replace(
          "{overall_risk}",
          overall_risk,
        );
      ...
      const interest_rate_rag_llm_query_request_obj = {
        messages: [{ role: "user", content: interest_rate_rag_llm_query }],
      };
      const interest_rate_rag_llm_query_request = JSON.stringify(
        interest_rate_rag_llm_query_request_obj,
      );
      console.log("payload:", interest_rate_rag_llm_query_request);

      const interest_rate_rag_llm_response = await makePostRequestCallByType(
        "get_interest_rate_from_rag_llm",
        interest_rate_rag_llm_query_request,
      )
        .then((responseData) => {
          console.log("Response:", responseData);
          return responseData;
      ...
      console.log(
        "interest_rate_rag_llm_response:",
        interest_rate_rag_llm_response,
      );
      //console.log("choices[]:", interest_rate_rag_llm_response.choices);
      console.log(
        "interest_rate_rag_llm_response.choices[0].message.content:",
        interest_rate_rag_llm_response.choices[0].message.content,
      );
  ...
  console.log(
    "Checking process.env.ENABLE_RAG_LLM for RAG LLM use in tool.",
    process.env.ENABLE_RAG_LLM,
  );
  if (process.env.ENABLE_RAG_LLM.toLowerCase() === "true") {
...
const setupModelWithTools = async (tools: Array<any>) => {
  console.log(
    `Using Amazon Bedrock model ${process.env.BEDROCK_MODEL} at ${process.env.BEDROCK_OPENAI_BASE_URL}`,
  );
...
webapp.listen(
  ...
  () => {
    console.log(
      `Agentic AI application ${process.env.APPLICATION_NAME} is starting...`,
    );
    console.log(
      `Server is running on http://${process.env.APPLICATION_HOST}:${process.env.APPLICATION_PORT}`,
    );
  },
);
